...
func TestDemo(t *testing.T) {
	...
	engine.ServeHTTP(w, newPostRequest("demos", addItem))
	...
	// get /demos?type=page
	engine.ServeHTTP(w, newGetRequest("demos",
	...
	putItem.Name = "测试用例2"
	engine.ServeHTTP(w, newPutRequest("demos/%s", putItem, addNewItem.RecordID))
	assert.Equal(t, 200, w.Code)
	releaseReader(w.Body)

	// get /demos/:id
	engine.ServeHTTP(w, newGetRequest("demos/%s", nil, addNewItem.RecordID))
	assert.Equal(t, 200, w.Code)
	...
	// delete /demos/:id
	engine.ServeHTTP(w, newDeleteRequest("demos/%s", addNewItem.RecordID))
	assert.Equal(t, 200, w.Code)
	releaseReader(w.Body)
}
