...
type MenuActionResource struct {
	...
	ActionID string `gorm:"column:action_id;size:36;index;not null;"` // 菜单动作ID
	Method   string `gorm:"column:method;size:100;not null;"`         // 资源请求方式(支持正则)
	Path     string `gorm:"column:path;size:100;not null;"`           // 资源请求路径（支持/:id匹配）
}
