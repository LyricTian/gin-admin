...
type User struct {
	...
	UserName string  `gorm:"column:user_name;size:64;index;not null;"` // 用户名
	RealName string  `gorm:"column:real_name;size:64;index;not null;"` // 真实姓名
	Password string  `gorm:"column:password;size:40;not null;"`        // 密码(sha1(md5(明文))加密)
