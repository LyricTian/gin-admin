...
func RateLimiterMiddleware(skippers ...SkipperFunc) gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		...
		name := contextx.FromUserID(c.Request.Context())
		if name == "" {
			name = c.ClientIP()
		}
		...
		if name != "" {
			limit := cfg.Count
			rate, delay, allowed := limiter.AllowMinute(name, limit)
			if !allowed {
				h := c.Writer.Header()
				...
				ginx.ResError(c, errors.TooManyRequests(consts.ErrTooManyRequestsID, ""))
				return
			}
		}
